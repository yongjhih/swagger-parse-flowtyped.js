...
let getViewForSwagger2 = function (opts) {
  ...
  _.forEach(swagger.definitions, function (swaggerDefinition, name) {
    ...
    _.forEach(swaggerDefinition.properties, (swaggerProperty, propertyName) => {
      ...
      property.required = propertyName.includes(swaggerDefinition.required);
      property.cardinality = property.required ? '' : '?';
      if (swaggerProperty.type === 'array') {
        if (swaggerProperty.items.type) {
          if (swaggerProperty.items.type === 'integer') {
            property.type = `void | ?Array<${property.cardinality}number>`;
          } else {
            property.type = `void | ?Array<${property.cardinality}${swaggerProperty.items.type}>`;
          }
        } else if (swaggerProperty.items['$ref']) {
          property.type = `void | ?Array<${property.cardinality}${swaggerProperty.items['$ref'].replace(/^#\/definitions\//, '')}>`;
        }
      } else {
        if (swaggerProperty.type) {
          if (swaggerProperty.type === 'integer') {
            property.type = `${property.required ? '' : "void | "}${property.cardinality}number`;
          } else {
            property.type = `${property.required ? '' : "void | "}${property.cardinality}${swaggerProperty.type}`;
          }
        } else if (swaggerProperty['$ref']) {
          property.type = `${property.required ? '' : "void | "}${property.cardinality}${swaggerProperty['$ref'].replace(/^#\/definitions\//, '')}`;
        }
      }
